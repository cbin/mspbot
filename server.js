...
dialog.matches(/^search/i, [
    function (session, args, next) {
        if (session.message.text.toLowerCase() == 'search') {
            builder.Prompts.text(session, 'Кого ищем?');
        } else {
    ...
    function (session, result, next) {
        ...
        if (!query) {
            ...
            githubClient.executeSearch(query, function (profiles) {
                ...
                if (totalCount == 0) {
                    session.endDialog('Извините, ни кого не нашли.');
                    ...
                    builder.Prompts.choice(session, 'Какого пользователя вы хотите посмотреть?', usernames);
                }
            });
        }
    }, function (session, result, next) {
